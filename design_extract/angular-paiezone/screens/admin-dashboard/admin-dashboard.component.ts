...
@Component({
    ...
    <div class="pz-page">
      ...
      <div class="stat-grid">
        <div class="pz-card stat">
          <div class="stat-head"><span class="ico"><pz-icon name="Users"/></span>Utilisateurs actifs</div>
          <div class="stat-val">{{ activeUsers() }}<small>/ {{ data.tenantUsers().length }}</small></div>
          <div class="stat-foot pz-muted">{{ data.tenantUsers().length - activeUsers() }} désactivé(s)</div>
        </div>
        <div class="pz-card stat">
          <div class="stat-head"><span class="ico info"><pz-icon name="Briefcase"/></span>Effectif total</div>
        ...
        <div class="pz-card stat">
          <div class="stat-head"><span class="ico warn"><pz-icon name="Shield"/></span>2FA activée</div>
          <div class="stat-val">{{ twofaPct() }}<small>%</small></div>
          <div class="stat-foot pz-muted">Recommandé pour Admin & RH</div>
        </div>
        <div class="pz-card stat">
          <div class="stat-head"><span class="ico pos"><pz-icon name="Wallet"/></span>Plan d'abonnement</div>
      ...
      <div class="grid">
        <div class="pz-card">
          <div class="card-head">
            ...
            <a class="pz-btn pz-sm pz-ghost" routerLink="/paiezone/admin-users">Gérer <pz-icon name="Arrow" [size]="12" [strokeWidth]="1.6"/></a>
          </div>
          <div class="card-body">
        ...
        <div class="pz-card">
          ...
          <div class="card-body">
            ...
            <div class="info-rows">
              ...
              <div class="info-row"><span class="pz-muted small">Effectif</span><span class="strong">{{ data.employees().length }} collaborateurs</span></div>
  ...
  styles: [`
    ...
    .stat-grid { display: grid; gap: var(--pz-gap); grid-template-columns: repeat(4, 1fr); }
    .stat { padding: 18px 20px; }
    .stat-head { display: flex; align-items: center; gap: 8px; color: var(--pz-muted); font-size: 12px; font-weight: 500; margin-bottom: 8px; }
    .stat-head .ico { width: 30px; height: 30px; border-radius: 8px; background: var(--pz-primary-soft); color: var(--pz-primary); display: grid; place-items: center; }
    .stat-head .ico.info { background: var(--pz-info-soft); color: var(--pz-info-ink); }
    .stat-head .ico.warn { background: var(--pz-warn-soft); color: var(--pz-warn-ink); }
    .stat-head .ico.pos { background: var(--pz-pos-soft); color: var(--pz-pos-ink); }
    .stat-val { font-size: 26px; font-weight: 600; letter-spacing: -0.025em; font-variant-numeric: tabular-nums; line-height: 1.1; }
    .stat-val small { font-size: 14px; color: var(--pz-muted); font-weight: 500; margin-left: 3px; }
    .stat-foot { display: flex; gap: 8px; margin-top: 10px; font-size: 12px; align-items: center; }
    .grid { display: grid; gap: var(--pz-gap); grid-template-columns: 2fr 1fr; }
    .card-head { display: flex; align-items: center; padding: 16px 20px 12px; }
    .card-title { font-size: 14px; font-weight: 600; }
    .card-head .pz-btn { margin-left: auto; }
    .card-body { padding: 4px 20px 20px; }
    .role-row { margin-bottom: 14px; }
    .row-top { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
    .row-top .pz-mono { margin-left: auto; font-size: 13px; }
    .small { font-size: 12px; }
    .strong { font-weight: 500; }
    .progress { height: 4px; background: var(--pz-surface-3); border-radius: 999px; overflow: hidden; }
    .progress i { display: block; height: 100%; background: var(--pz-primary); border-radius: 999px; }
    .company-head { display: flex; gap: 14px; align-items: center; margin-bottom: 14px; }
    .company-head .logo { width: 56px; height: 56px; border-radius: 12px; background: linear-gradient(135deg, var(--pz-primary), #7c3aed); color: #fff; display: grid; place-items: center; font-weight: 700; font-size: 18px; }
    .info-rows { margin-bottom: 14px; }
    .info-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid var(--pz-line); }
