    ...
    <div className="page">
      ...
      <div className="grid c2">
        ...
        <div className="card">
          <div className="card-head">
            <div>
              <div className="card-title">Échéances & événements à venir</div>
              <div className="card-sub">30 prochains jours · actions à anticiper</div>
            </div>
            <button className="btn sm ghost">Tout voir <Ico.Arrow/></button>
          </div>
          <div className="card-body" style={{padding:'0 12px 14px'}}>
            {UPCOMING.map((u, i) => {
              const e = empById(u.empId);
              const meta = UPCOMING_META[u.kind];
              const I = Ico[meta.icon] || Ico.Calendar;
              return (
                <div key={i} className="row" style={{padding:'10px 8px',borderTop: i===0?'0':'1px solid var(--line)',gap:12,alignItems:'center'}}>
                  <div className={`pill ${meta.tone}`} style={{width:32,height:32,padding:0,borderRadius:8,justifyContent:'center',flex:'0 0 32px'}}>
                    <I/>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div className="row" style={{gap:6,alignItems:'center'}}>
                      <span style={{fontSize:13,fontWeight:500,color:'var(--ink)'}}>{fullName(e)}</span>
                      <span className={`pill ${meta.tone}`} style={{fontSize:10,padding:'1px 6px',height:18}}>{meta.short}</span>
                      {u.urgent && <span className="pill warn" style={{fontSize:10,padding:'1px 6px',height:18}}>● Urgent</span>}
                    </div>
                  ...
                  <div style={{textAlign:'right',flex:'0 0 auto'}}>
                    <div style={{fontSize:12,fontWeight:600,color:u.urgent?'var(--warn-ink)':'var(--ink-2)',fontFamily:'JetBrains Mono, monospace'}}>{u.when}</div>
                    <div className="muted" style={{fontSize:10.5,marginTop:1}}>{u.date}</div>
                  </div>
                  {u.action && (
                    <button className="btn sm" style={{flex:'0 0 auto'}}>{u.action}</button>
                  )}
                </div>
              );
            })}
