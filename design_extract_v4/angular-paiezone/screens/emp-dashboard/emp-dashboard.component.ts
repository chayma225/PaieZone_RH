...
@Component({
          ...
          @for (b of balances; track b.label) {
            <div class="bal">
              <div class="bal-head">
                ...
                <span><strong class="pz-mono">{{ b.total - b.used }}</strong> <span class="pz-muted small">/ {{ b.total }}j restants</span></span>
              </div>
  ...
  styles: [`
    ...
    .grid-21 { display: grid; gap: var(--pz-gap); grid-template-columns: 2fr 1fr; }

    .hero {
      background: linear-gradient(135deg, #ffffff 0%, var(--pz-primary-soft) 100%);
      border-color: var(--pz-primary-soft);
    }
    .hero-head { padding: 16px 20px 12px; display: flex; align-items: center; }
    .hero-head > div:first-child { flex: 1; }
    .hero-title { font-size: 14px; font-weight: 600; color: var(--pz-primary-ink); }
    .hero-sub { font-size: 12px; color: var(--pz-muted); margin-top: 2px; }
    .hero-body { padding: 4px 20px 20px; display: flex; justify-content: space-between; align-items: flex-end; gap: 20px; flex-wrap: wrap; }
    .amount-label { font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; color: var(--pz-muted); }
    .amount { font-size: 36px; font-weight: 600; letter-spacing: -0.025em; font-family: 'JetBrains Mono', monospace; color: var(--pz-ink); margin-top: 4px; }
    .amount-detail { font-size: 12px; margin-top: 6px; }
    .hero-actions { display: flex; gap: 8px; }

    .card-head { padding: 16px 20px 12px; }
    .card-title { font-size: 14px; font-weight: 600; }
    .card-body { padding: 0 20px 14px; }

    .notif { padding: 10px 0; border-top: 1px solid var(--pz-line); display: flex; gap: 12px; align-items: flex-start; }
    .notif:first-child { border-top: 0; }
    .notif-ico {
      width: 32px; height: 32px; border-radius: 8px;
      display: grid; place-items: center;
      background: var(--pz-primary-soft); color: var(--pz-primary-ink);
      flex-shrink: 0;
    }
    .notif-ico.pos { background: var(--pz-pos-soft); color: var(--pz-pos-ink); }
    .notif-ico.warn { background: var(--pz-warn-soft); color: var(--pz-warn-ink); }
    .notif-ico.info { background: var(--pz-info-soft); color: var(--pz-info-ink); }
    .grow { flex: 1; min-width: 0; }
    .strong { font-weight: 500; font-size: 13px; }
    .small { font-size: 11.5px; }

    .bal { margin-bottom: 14px; }
    .bal-head { display: flex; justify-content: space-between; margin-bottom: 6px; }
    .progress { height: 4px; background: var(--pz-surface-3); border-radius: 999px; overflow: hidden; }
    .progress i { display: block; height: 100%; border-radius: 999px; transition: width 0.3s; }
  `],
