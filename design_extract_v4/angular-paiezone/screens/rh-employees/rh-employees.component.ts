...
export default class RhEmployeesComponent {
  ...
  protected readonly depts = computed(() => ['Tous', ...new Set(this.data.employees().map(e => e.dept))]);
  ...
  protected readonly filtered = computed(() => {
    ...
    return this.data.employees().filter(e => {
      ...
      if (q && !this.data.fullName(e).toLowerCase().includes(q)
          && !e.matricule.toLowerCase().includes(q)) return false;
      return true;
    });
  });
  ...
  pickFiles(): void { this.fileInput()?.nativeElement.click(); }
  ...
  private handleFiles(files: FileList): void {
    ...
    arr.forEach((f, i) => {
      ...
      // Simulate upload progress. In production: HttpClient.post with reportProgress.
      let p = 0;
      const tick = setInterval(() => {
        ...
        if (p >= 100) {
          ...
          this.uploading.update(u => u.map(x =>
            x.id === id ? { ...x, progress: Math.min(100, p) } : x,
          ));
        }
      }, 120);
    });
