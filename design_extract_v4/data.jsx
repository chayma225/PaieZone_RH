...
// ─── Échéances & événements à venir (RH dashboard) ───────────────────
// Données calculées depuis les employés — anniversaires, fins d'essai, CDD…
const UPCOMING = [
  { kind: 'trial-end', empId: 8, label: 'Fin de période d\'essai', when: 'Dans 3 jours', date: '17 mai', urgent: true, action: 'Décider' },
  { kind: 'birthday', empId: 2, label: 'Anniversaire', when: 'Demain', date: '15 mai', sub: '32 ans', action: 'Envoyer un mot' },
  { kind: 'cdd-end', empId: 10, label: 'Fin de CDD', when: 'Dans 12 jours', date: '26 mai', urgent: true, action: 'Renouveler ?' },
  { kind: 'anniv', empId: 1, label: 'Anniversaire de contrat', when: 'Dans 18 jours', date: '02 juin', sub: '8 ans d\'ancienneté' },
  { kind: 'medical', empId: 4, label: 'Visite médicale annuelle', when: 'Dans 22 jours', date: '06 juin', action: 'Planifier' },
  { kind: 'birthday', empId: 9, label: 'Anniversaire', when: 'Dans 26 jours', date: '10 juin', sub: '35 ans' },
];

// Mapping kind → { icon, tone }
const UPCOMING_META = {
  'trial-end':  { icon: 'CircleAlert', tone: 'warn',    short: 'Essai' },
  'cdd-end':    { icon: 'Briefcase',   tone: 'warn',    short: 'CDD' },
  'birthday':   { icon: 'Cake',        tone: 'primary', short: 'Anniv.' },
  'anniv':      { icon: 'Sparkles',    tone: 'pos',     short: 'Ancienneté' },
  'medical':    { icon: 'Shield',      tone: 'info',    short: 'Médical' },
};
...
Object.assign(window, {
  ...
  ACTIVITY, UPCOMING, UPCOMING_META, SAAS_KPI, SAAS_MRR_SERIES, RH_PAYROLL_SERIES, CAL_MAY,
