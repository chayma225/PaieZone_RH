...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Amiel_Simon', email: 'Libere.Robert65@gmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Edgard81');
      cy.get(`[data-cy="login"]`).should('have.value', 'Edgard81');

      cy.get(`[data-cy="firstName"]`).type('Anatolie');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Anatolie');

      cy.get(`[data-cy="lastName"]`).type('Guyot');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Guyot');

      cy.get(`[data-cy="email"]`).type('Scholastique50@yahoo.fr');
      cy.get(`[data-cy="email"]`).should('have.value', 'Scholastique50@yahoo.fr');
