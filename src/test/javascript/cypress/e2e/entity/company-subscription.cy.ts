...
describe('CompanySubscription e2e test', () => {
  ...
  describe('new CompanySubscription page', () => {
    ...
    it('should create an instance of CompanySubscription', () => {
      cy.get(`[data-cy="plan"]`).select('BUSINESS');

      cy.get(`[data-cy="status"]`).select('CANCELLED');

      cy.get(`[data-cy="maxEmployees"]`).type('17207');
      cy.get(`[data-cy="maxEmployees"]`).should('have.value', '17207');

      cy.get(`[data-cy="priceHT"]`).type('15382.21');
      cy.get(`[data-cy="priceHT"]`).should('have.value', '15382.21');

      cy.get(`[data-cy="billingDay"]`).type('19331');
      cy.get(`[data-cy="billingDay"]`).should('have.value', '19331');
      ...
      cy.get(`[data-cy="renewalDate"]`).type('2026-03-31');
      cy.get(`[data-cy="renewalDate"]`).blur();
      cy.get(`[data-cy="renewalDate"]`).should('have.value', '2026-03-31');

      cy.get(`[data-cy="notes"]`).type('géométrique au lieu de hors');
      cy.get(`[data-cy="notes"]`).should('have.value', 'géométrique au lieu de hors');
