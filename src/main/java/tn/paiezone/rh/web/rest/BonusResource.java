...
import tn.paiezone.rh.service.BonusQueryService;
...
/**
 * REST controller for managing {@link tn.paiezone.rh.domain.Bonus}.
 */
@RestController
...
public class BonusResource {
    ...
    public ResponseEntity<BonusDTO> createBonus(@Valid @RequestBody BonusDTO bonusDTO) throws URISyntaxException {
        ...
        bonusDTO = bonusService.save(bonusDTO);
        return ResponseEntity.created(new URI("/api/bonuses/" + bonusDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, bonusDTO.getId().toString()))
            .body(bonusDTO);
    }
    ...
    public ResponseEntity<BonusDTO> updateBonus(
        ...
        bonusDTO = bonusService.update(bonusDTO);
