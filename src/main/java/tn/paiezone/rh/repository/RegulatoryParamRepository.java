...
public interface RegulatoryParamRepository extends JpaRepository<RegulatoryParam, Long> {
    ...
    @Modifying
    @Query(
        "UPDATE RegulatoryParam p SET p.active = false, p.effectiveTo = :endDate " +
    ...
    @Modifying
    @Query("UPDATE RegulatoryParam p SET p.numericValue = :value WHERE p.paramKey = :key AND p.active = true")
    int updateValueByKey(@Param("key") String key, @Param("value") java.math.BigDecimal value);

    /** SQL natif — bypass total du cache Hibernate L2 */
    @Modifying
    @Query(value = "UPDATE regulatory_param SET numeric_value = :value WHERE param_key = :key AND active = true", nativeQuery = true)
    int updateValueByKeyNative(@Param("key") String key, @Param("value") java.math.BigDecimal value);
