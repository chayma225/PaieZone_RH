 ...
 * @howToUse
 * ```
 *     <some-element *pzHasAnyAuthority="'ROLE_ADMIN'">...</some-element>
 *
 *     <some-element *pzHasAnyAuthority="['ROLE_ADMIN', 'ROLE_USER']">...</some-element>
 * ```
 */
@Directive({
  selector: '[pzHasAnyAuthority]',
})
export default class HasAnyAuthorityDirective {
  readonly authorities = input<string | string[]>([], { alias: 'pzHasAnyAuthority' });
