...
import { sampleWithRequiredData } from '../hr-document.test-samples';
import { HrDocumentService } from '../service/hr-document.service';

import { HrDocument } from './hr-document';

vitest.useFakeTimers();

describe('HrDocument Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: HrDocument;
  let fixture: ComponentFixture<HrDocument>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(HrDocument);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.hrDocuments()[0]).toEqual(expect.objectContaining({ id: 28321 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 28321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.hrDocuments()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 28321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to hrDocumentService', () => {
      const entity = { id: 28321 };
      vitest.spyOn(service, 'getHrDocumentIdentifier');
      const id = comp.trackId(entity);
      expect(service.getHrDocumentIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.hrDocumentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
