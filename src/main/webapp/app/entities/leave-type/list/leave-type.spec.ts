...
import { sampleWithRequiredData } from '../leave-type.test-samples';
...
describe('LeaveType Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 22862 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.leaveTypes()[0]).toEqual(expect.objectContaining({ id: 22862 }));
  });

  describe('trackId', () => {
    it('should forward to leaveTypeService', () => {
      const entity = { id: 22862 };
      vitest.spyOn(service, 'getLeaveTypeIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.leaveTypesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
