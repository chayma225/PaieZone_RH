...
import { sampleWithRequiredData } from '../payroll-period.test-samples';
...
describe('PayrollPeriod Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PayrollPeriod;
  let fixture: ComponentFixture<PayrollPeriod>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PayrollPeriod);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28456 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.payrollPeriods()[0]).toEqual(expect.objectContaining({ id: 28456 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 28456 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.payrollPeriods()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 28456 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to payrollPeriodService', () => {
      ...
      vitest.spyOn(service, 'getPayrollPeriodIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPayrollPeriodIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.payrollPeriodsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
