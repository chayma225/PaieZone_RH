...
import { sampleWithRequiredData } from '../public-holiday.test-samples';
...
describe('PublicHoliday Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 12391 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.publicHolidays()[0]).toEqual(expect.objectContaining({ id: 12391 }));
  });

  describe('trackId', () => {
    it('should forward to publicHolidayService', () => {
      const entity = { id: 12391 };
      vitest.spyOn(service, 'getPublicHolidayIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPublicHolidayIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.publicHolidaysParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
