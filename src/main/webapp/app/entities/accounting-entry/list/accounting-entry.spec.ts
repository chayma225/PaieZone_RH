...
import { sampleWithRequiredData } from '../accounting-entry.test-samples';
...
describe('AccountingEntry Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AccountingEntry;
  let fixture: ComponentFixture<AccountingEntry>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AccountingEntry);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 1531 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.accountingEntries()[0]).toEqual(expect.objectContaining({ id: 1531 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 1531 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.accountingEntries()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 1531 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to accountingEntryService', () => {
      const entity = { id: 1531 };
      vitest.spyOn(service, 'getAccountingEntryIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAccountingEntryIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.accountingEntriesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
