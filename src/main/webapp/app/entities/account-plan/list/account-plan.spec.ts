...
import { sampleWithRequiredData } from '../account-plan.test-samples';
...
describe('AccountPlan Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 7481 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.accountPlans()[0]).toEqual(expect.objectContaining({ id: 7481 }));
  });

  describe('trackId', () => {
    it('should forward to accountPlanService', () => {
      const entity = { id: 7481 };
      vitest.spyOn(service, 'getAccountPlanIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAccountPlanIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.accountPlansParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
