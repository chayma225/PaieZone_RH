...
import { sampleWithRequiredData } from '../leave-balance.test-samples';
...
describe('LeaveBalance Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9581 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.leaveBalances()[0]).toEqual(expect.objectContaining({ id: 9581 }));
  });

  describe('trackId', () => {
    it('should forward to leaveBalanceService', () => {
      const entity = { id: 9581 };
      vitest.spyOn(service, 'getLeaveBalanceIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.leaveBalancesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
