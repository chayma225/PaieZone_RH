...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'birthDate' | 'hireDate' | 'trialEndDate' | 'createdAt' | 'updatedAt'> & {
...
export class EmployeesService {
  ...
  protected convertValueFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...
      trialEndDate: restEmployee.trialEndDate ? dayjs(restEmployee.trialEndDate) : undefined,
...
export class EmployeeService extends EmployeesService {
  ...
  protected convertValueFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...
      trialEndDate: employee.trialEndDate?.format(DATE_FORMAT) ?? null,
      createdAt: employee.createdAt?.toJSON() ?? null,
      updatedAt: employee.updatedAt?.toJSON() ?? null,
    };
  }
