...
import { sampleWithRequiredData } from '../official-document.test-samples';
...
describe('OfficialDocument Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: OfficialDocument;
  let fixture: ComponentFixture<OfficialDocument>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(OfficialDocument);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 18816 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.officialDocuments()[0]).toEqual(expect.objectContaining({ id: 18816 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 18816 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.officialDocuments()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 18816 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to officialDocumentService', () => {
      const entity = { id: 18816 };
      vitest.spyOn(service, 'getOfficialDocumentIdentifier');
      const id = comp.trackId(entity);
      expect(service.getOfficialDocumentIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.officialDocumentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.officialDocumentsParams()).toMatchObject(
